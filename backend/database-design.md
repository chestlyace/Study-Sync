# StudySync - Complete Database Design
...
9. [Data Archival Strategy](#data-archival-strategy)
...
- **Expected Scale:** 50,000 users, 500,000 sessions/year

### Design Principles
1. **Normalization:** 3NF with strategic denormalization for performance
2. **Soft Deletes:** Critical data (users, groups) use soft deletes
3. **Audit Trail:** Important actions logged in audit_logs
4. **Timestamps:** All tables have created_at/updated_at
5. **UUIDs:** Primary keys use UUIDs for distributed systems
6. **JSONB:** Flexible data stored as JSONB with GIN indexes
...
└─────────────────────────────────────────────────────────────────────┘
                    ...
                    └──────┬───────┘
                           ...
                           ▼
    ┌──────────────┐   ┌──────────────┐   ┌──────────────┐
    │   courses    │◄──┤    users     │──►│  user_stats  │
    └──────┬───────┘   └──────┬───────┘   └──────────────┘
           │                  │
           │ N:M              │ 1:N
           ▼                  ▼
    ┌──────────────┐   ┌──────────────┐
    │user_courses  │   │study_groups  │
    └──────────────┘   └──────┬───────┘
                              │
                              │ 1:N
                    ┌─────────┼─────────┐
                    ...
                    ▼         ▼         ▼
            ┌──────────┐ ┌──────────┐ ┌──────────┐
            │ group_   │ │  group_  │ │ session_ │
            │ members  │ │ messages │ │ ratings  │
            └──────────┘ └──────────┘ └──────────┘
...
└─────────────────────────────────────────────────────────────────────┘
    ...
    ┌──────────────┐   ┌──────────────┐   ┌──────────────┐
    │ group_       │   │notifications │   │transactions  │
    │ materials    │   └──────────────┘   └──────────────┘
    └──────────────┘

    ┌──────────────┐   ┌──────────────┐   ┌──────────────┐
    │ message_     │   │  otp_codes   │   │ refresh_     │
    │ reads        │   └──────────────┘   │ tokens       │
    └──────────────┘                       └──────────────┘
...
This is the complete database schema. Would you like me to continue with:
1. Sample queries and usage patterns
